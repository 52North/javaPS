...
import org.n52.iceland.ogc.wps.data.InMemoryProcessData;
...
public class WPSResponseWriterTest {
    ...
    public void test() throws OwsExceptionReport {
        ...
        group.addElement(new InMemoryProcessData(new OwsCode("output4"), new Format("application/json"), "{\"soamekey\": \"somevalue\"}".getBytes(StandardCharsets.UTF_8)));
        group.addElement(new InMemoryProcessData(new OwsCode("output5"), new Format("application/xml", "ISO-8859-1"), "<sömekey>somevalüe</sömekey>".getBytes(StandardCharsets.ISO_8859_1)));
        group.addElement(new InMemoryProcessData(new OwsCode("output6"), new Format("application/xml"), "<sömekey>somevalüe</sömekey>".getBytes(StandardCharsets.UTF_8)));
        result.addOutput(new InMemoryProcessData(new OwsCode("output7"), new Format("application/xml").withBase64Encoding(), BaseEncoding.base64().encode("<sömekey>somevalüe</sömekey>".getBytes(StandardCharsets.UTF_8)).getBytes(Charsets.US_ASCII)));
